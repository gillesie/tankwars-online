...
export function startGameClient() {
    ...
    // Only show scoreboard in MP/SP, hide in Campaign
...
function drawMinimap() {
    ...
    state.platforms.forEach(p => {
         ...
         ctx.moveTo(p.x * scaleX, projectY(p.y));
         ctx.lineTo((p.x + p.width) * scaleX, projectY(p.y + (p.width * Math.tan(p.angle * Math.PI/180))));
    });
    ctx.stroke();

    if (state.player) {
        ctx.fillStyle = state.player.team === 1 ? '#0ff' : '#f00';
        ctx.fillRect(state.player.x * scaleX, projectY(state.player.y), 4, 4);
    }
    
    Object.values(state.remotePlayers).forEach(e => {
        if(!e.dead) {
            ctx.fillStyle = e.team === 1 ? '#0ff' : '#f00';
            ctx.fillRect(e.x * scaleX, projectY(e.y), 4, 4);
        }
    });

    ctx.fillStyle = '#ff0';
    state.projectiles.forEach(p => { if (p.active) ctx.fillRect(p.x * scaleX, projectY(p.y), 2, 2); });

    ctx.fillStyle = '#667';
    state.blocks.forEach(b => { ctx.fillRect(b.x * scaleX, projectY(b.y), 2, 2); });

    ctx.fillStyle = '#fff';
    state.planes.forEach(p => { ctx.fillRect(p.x * scaleX, projectY(p.y), 5, 3); });

    state.crates.forEach(c => { 
        if (c.type === 'repair') ctx.fillStyle = '#0f0';
        else if (c.type === 'extra_life') ctx.fillStyle = '#fff';
        else if (c.type === 'builder') ctx.fillStyle = '#a0a';
        else ctx.fillStyle = '#f0f';
        ctx.fillRect((c.x * scaleX)-1, projectY(c.y)-1, 3, 3); 
    });
}
...
function animate() {
    ...
    if (state.gameActive && state.player) {
        ...
        if (state.gameMode === 'campaign') maxW = state.levelWidth; 
